...
![1](E:\sjtu\Computer_Organization\Hardware\pipline2\report\1.png)
...
![3](E:\sjtu\Computer_Organization\Hardware\pipline2\report\3.png)
...
![2](E:\sjtu\Computer_Organization\Hardware\pipline2\report\2.png)

从整体上来看本次实验三个端口的表现正常。现在进入细节部分查看关于数据冒险和控制冒险是否被解决。由于整体运行结果正常，因此我们可以人围除去 $lw$  造成的数据冒险和控制冒险之外，余下部分没有问题。对于跳转产生的控制冒险问题，通过查看如下具体波形

![4](E:\sjtu\Computer_Organization\Hardware\pipline2\report\4.png)可以看见在280ns处，$beq$ 指令$(10890005)$后方虽然紧随着 $add$ 指令$(00853020)$ ，但该指令并无发挥任和实质性作用，随后指令跳转至减法分支，执行减法指令$(00853022)$ 说明该CPU在解决由 $bne$，$beq$ 和 $j$ 跳转产生的控制冒险时并无问题。
...
![5](E:\sjtu\Computer_Organization\Hardware\pipline2\report\5.png)

##### 前半部分

![6](https://github.com/zengkaipeng/EI332/blob/master/Pipeline_CPU/report/6.png)
